...
class WeatherListFragment : Fragment() {
    ...
    private fun renderData(appState: AppState){
        when(appState){
            is AppState.Error -> {
                Toast.makeText(requireContext(), "Ошибка загрузки данных данных", Toast.LENGTH_SHORT).show()
            }
            is AppState.loading -> {
                Toast.makeText(requireContext(), "Выполняется загрузка данных", Toast.LENGTH_SHORT).show()
            }
            is AppState.SuccessOne -> {
                val result = appState.weatherData
            }
            is AppState.SuccessMulti -> {
                binding.fragmentRecyclerView.adapter = WeatherListAdapter(appState.weatherListData)
            }
